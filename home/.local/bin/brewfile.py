...
class BrewfileManager:
    ...
    def cmd_check(self, args):
        ...
        file_basenames = [p.name for p in all_managed_files]
        say(f"Checking system state against: {', '.join(file_basenames)}")
        declared_packages = {}
            ...
            if file_path.exists():
                with open(file_path, 'r') as f:
                    for line in f:
                        ...
                        if cleaned_line and cleaned_line not in declared_packages:
                            declared_packages[cleaned_line] = file_path.name
        installed_set = set()
        ...
        declared_set_keys = set(declared_packages.keys())
        to_install = sorted(list(declared_set_keys - installed_set))
        to_add_to_brewfile = sorted(list(installed_set - declared_set_keys))
        if not to_install and not to_add_to_brewfile:
            say("System state matches Brewfile(s). No changes needed.")
            return
        print()
        print(f"    {YELLOW}Missing from System{RESET}")
        self._group_print(to_install, package_origins=declared_packages)
        print()
        print(f"    {YELLOW}Missing in Brewfile{RESET}")
        self._group_print(to_add_to_brewfile)
    ...
    def cmd_list(self, args):
        ...
        self._ensure_brew()
        files_to_list = [self.brewfile] + self._resolve_include_files(args)
        all_deps = set()
    ...
    def cmd_remove(self, args):
        ...
        found_file = None
        line_to_remove = None
        package_re = re.compile(fr'^(?:tap|brew|cask|mas|whalebrew|vscode)\s+\"{re.escape(package_name)}\"')
        ...
        if choice == '1':
            ...
            if uninstall_result.returncode == 0:
                say(f"Uninstall successful. Removing '{line_to_remove}' from {found_file.name}...")
                self._remove_line_from_file(found_file, line_to_remove)
                say(f"'{package_name}' has been uninstalled and removed from your configuration.")
            else:
                warn(f"Failed to uninstall '{package_name}'. Your Brewfile has not been changed.")
        elif choice == '2':
...
def main():
    ...
    for cmd, help_text in commands.items():
        ...
        if cmd == "add":
            ...
            subparser.add_argument(
                "--to", type=str, help="Short name of the Brewfile to add to (e.g., 'extra')."
            )
        if cmd == "remove":
